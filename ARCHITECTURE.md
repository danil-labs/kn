...
Los documentos que siguen en la nube se reconocen por metadatos en [cloud.rs](crates/kn-core/src/cloud.rs) y, en cada llamada a `status` o `add`, se ocultan a Git con un archivo de exclusión temporal. Los patrones de ignore no alcanzan a los ya versionados: esos se marcan `assume-unchanged` en el índice, y cada llamada reconcilia las marcas con lo que sigue en la nube, así que la quita en cuanto se descargan. `status --porcelain`, con el lock compartido, las aplica sobre una copia temporal del índice. La lista de exclusión no se guarda: el directorio Git común lo comparten la principal y las sesiones, y una exclusión guardada ahí ocultaría en una sesión un documento nuevo del agente. En builds de depuración, `KN_TEST_CLOUD_ONLY` simula esos documentos por nombre para las regresiones. Si `init` falla, borra el historial que acababa de crear en KN_HOME.
