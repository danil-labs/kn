...
| Una copia con marcador anterior no opera silenciosamente sobre el historial de una ubicación todavía existente | `copies_moves_locks_and_invalid_input` |
| Ni las lecturas ni las escrituras crean archivos de kn en la carpeta de documentos: la identidad y el lock de init viven en KN_HOME | `special_names_ignores_empty_folders_and_read_only_queries`, `init_writes_nothing_into_the_documents_folder` |
| La identidad sale del registro: ni un historial huérfano ni el anterior a `--fresh` que apuntan a la carpeta se eligen | `orphan_histories_are_ignored` |
| Una principal registrada dentro de otra tiene su propio historial; lo que integra llega a la de arriba como `external_observation` | `nested_primaries_keep_independent_histories` |
| No se inicializa dentro de una sesión | `init_inside_a_session_is_refused` |
| Una principal con marcador anterior sigue funcionando y las lecturas no la registran; `migrate` registra antes de quitar, es idempotente y no quita nada si `.kn` tiene archivos ajenos | `legacy_markers_keep_working_and_migrate_removes_them` |
| Un marcador sincronizado cuyo historial no está en esta máquina no bloquea `init`, que no lo reescribe; `migrate` no lo quita | `a_synced_legacy_marker_does_not_block_another_machine` |
...
| Una sesión antigua no escribe una principal reinicializada, aunque la carpeta conserve su marcador anterior | `old_sessions_cannot_publish_to_a_reinitialized_primary` |
