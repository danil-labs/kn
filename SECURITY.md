...
- La identidad de una principal vive en KN_HOME (`roots.json`), no en la carpeta: una carpeta sincronizada no lleva archivos de kn a otras máquinas. Un `.kn/config.json` anterior que llega por sincronización es entrada no confiable: una carpeta registrada no lo lee, `init` no lo reescribe y `migrate` no lo quita si nombra otro historial.
- El registro asocia rutas, no contenido. Si otra carpeta ocupa una ruta registrada, kn la trata como esa principal y registra sus diferencias como cambios externos.
